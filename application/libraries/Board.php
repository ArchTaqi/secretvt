...
class Board extends CI_Controller
{
        ...
        if ($latest && is_array($latest)) {
            foreach ($latest as $key => $value) {
                $view['view']['latest'][$key]['name'] = display_username(
                    ...
                    (element('post_nickname', $value) ? element('post_nickname', $value) : element('post_username', $value))
                );
        ...
        if ($latest && is_array($latest)) {
            foreach ($latest as $key => $value) {
                $view['view']['latest'][$key]['name'] = display_username(
                    ...
                    (element('cmt_nickname', $value) ? element('cmt_nickname', $value) : element('cmt_username', $value))
     ...
     * 추천 게시물을 가져옵니다
     */
    public function latest_like($config){
        ...
        if ($cache_minute> 0) {
            ...
            $cache_exclude_brd_id = is_array($exclude_brd_id) ? implode('-', $exclude_brd_id) : $exclude_brd_id;
            $cache_exclude_brd_key = is_array($exclude_brd_key) ? implode('-', $exclude_brd_key) : $exclude_brd_key;
            $cachename = 'latest/latest_like-s-' . $skin . '-i-' . $cache_brd_id . '-k-' . $cache_brd_key . '-l-' . $cache_exclude_brd_id . '-k-' . $cache_exclude_brd_key . '-l-' . $limit . '-t-' . $length . '-g-' . $is_gallery . '-w-' . $image_width . '-h-' . $image_height . '-p-' . $period_second;
            $html = $this->CI->cache->get($cachename);
            if ($html) {
                return $html;
            }
        ...
        $view['view']['skinurl'] = base_url( VIEW_DIR . 'latest/' . $skin);
        $html = $this->CI->load->view('latest/' . $skin . '/latest_like', $view, true);
     ...
     * 신규 컨텐츠 게시물을 가져옵니다
     */
    public function latest_new($config){
        ...
        if ($cache_minute> 0) {
            ...
            $cache_exclude_brd_id = is_array($exclude_brd_id) ? implode('-', $exclude_brd_id) : $exclude_brd_id;
            $cache_exclude_brd_key = is_array($exclude_brd_key) ? implode('-', $exclude_brd_key) : $exclude_brd_key;
            $cachename = 'latest/latest_new-s-' . $skin . '-i-' . $cache_brd_id . '-k-' . $cache_brd_key . '-l-' . $cache_exclude_brd_id . '-k-' . $cache_exclude_brd_key . '-l-' . $limit . '-t-' . $length . '-g-' . $is_gallery . '-w-' . $image_width . '-h-' . $image_height . '-p-' . $period_second;
            $html = $this->CI->cache->get($cachename);
            if ($html) {
                return $html;
            }
        ...
        $result = $this->CI->db->get();
        $view['view']['latest'] = array();
        $latest = $result->result_array();

        $view['view']['latest_limit'] = $limit;

        $i=0;
        $latest_new_limit=array();
        if ($latest && is_array($latest)) {
            foreach ($latest as $key => $value) {

                $brd_key = $this->CI->board->item_id('brd_key', element('brd_id', $value));
                if(!empty($latest_new_limit[$brd_key]) && count($latest_new_limit[$brd_key]) > 1) continue; 
                if($i > 5) break;
                $i++;
                
                $latest_new_limit[$brd_key][$key] = element('brd_id', $value);
                $view['view']['latest'][$key]['name'] = display_username(
                    element('post_userid', $value),
                    (element('post_nickname', $value) ? element('post_nickname', $value) : element('post_username', $value))
                );
        ...
        $view['view']['skinurl'] = base_url( VIEW_DIR . 'latest/' . $skin);
        $html = $this->CI->load->view('latest/' . $skin . '/latest_new', $view, true);
    ...
    public function latest_free($config){
        ...
        if ($cache_minute> 0) {
            ...
            $cache_exclude_brd_id = is_array($exclude_brd_id) ? implode('-', $exclude_brd_id) : $exclude_brd_id;
            $cache_exclude_brd_key = is_array($exclude_brd_key) ? implode('-', $exclude_brd_key) : $exclude_brd_key;
            $cachename = 'latest/latest_free-s-' . $skin . '-i-' . $cache_brd_id . '-k-' . $cache_brd_key . '-l-' . $cache_exclude_brd_id . '-k-' . $cache_exclude_brd_key . '-l-' . $limit . '-t-' . $length . '-g-' . $is_gallery . '-w-' . $image_width . '-h-' . $image_height . '-p-' . $period_second;
            $html = $this->CI->cache->get($cachename);
            if ($html) {
                return $html;
            }
        ...
        $view['view']['skinurl'] = base_url( VIEW_DIR . 'latest/' . $skin);
        $html = $this->CI->load->view('latest/' . $skin . '/latest_free', $view, true);
    ...
    public function latest_gallery($config){
        ...
        if ($cache_minute> 0) {
            ...
            $cache_exclude_brd_id = is_array($exclude_brd_id) ? implode('-', $exclude_brd_id) : $exclude_brd_id;
            $cache_exclude_brd_key = is_array($exclude_brd_key) ? implode('-', $exclude_brd_key) : $exclude_brd_key;
            $cachename = 'latest/latest_gallery-s-' . $skin . '-i-' . $cache_brd_id . '-k-' . $cache_brd_key . '-l-' . $cache_exclude_brd_id . '-k-' . $cache_exclude_brd_key . '-l-' . $limit . '-t-' . $length . '-g-' . $is_gallery . '-w-' . $image_width . '-h-' . $image_height . '-p-' . $period_second;
            $html = $this->CI->cache->get($cachename);
            if ($html) {
                return $html;
            }
        ...
        $view['view']['skinurl'] = base_url( VIEW_DIR . 'latest/' . $skin);
        $html = $this->CI->load->view('latest/' . $skin . '/latest_gallery', $view, true);
    ...
    public function latest_ltn($config){
        ...
        if ($cache_minute> 0) {
            ...
            $cache_exclude_brd_id = is_array($exclude_brd_id) ? implode('-', $exclude_brd_id) : $exclude_brd_id;
            $cache_exclude_brd_key = is_array($exclude_brd_key) ? implode('-', $exclude_brd_key) : $exclude_brd_key;
            $cachename = 'latest/latest_ltn-s-' . $skin . '-i-' . $cache_brd_id . '-k-' . $cache_brd_key . '-l-' . $cache_exclude_brd_id . '-k-' . $cache_exclude_brd_key . '-l-' . $limit . '-t-' . $length . '-g-' . $is_gallery . '-w-' . $image_width . '-h-' . $image_height . '-p-' . $period_second;
            $html = $this->CI->cache->get($cachename);
            if ($html) {
                return $html;
            }
        ...
        $view['view']['skinurl'] = base_url( VIEW_DIR . 'latest/' . $skin);
        $html = $this->CI->load->view('latest/' . $skin . '/latest_ltn', $view, true);
    ...
    public function latest_lately($config){
        ...
        if ($cache_minute> 0) {
            ...
            $cache_exclude_brd_id = is_array($exclude_brd_id) ? implode('-', $exclude_brd_id) : $exclude_brd_id;
            $cache_exclude_brd_key = is_array($exclude_brd_key) ? implode('-', $exclude_brd_key) : $exclude_brd_key;
            $cachename = 'latest/latest_lately-s-' . $skin . '-i-' . $cache_post_id . '-k-' . $cache_brd_key . '-l-' . $cache_exclude_brd_id . '-k-' . $cache_exclude_brd_key . '-l-' . $limit . '-t-' . $length . '-g-' . $is_gallery . '-w-' . $image_width . '-h-' . $image_height . '-p-' . $period_second;
            $html = $this->CI->cache->get($cachename);
            if ($html) {
                return $html;
            }
        ...
        $view['view']['skinurl'] = base_url( VIEW_DIR . 'latest/' . $skin);
        $html = $this->CI->load->view('latest/' . $skin . '/latest_lately', $view, true);
